    ...
    <div className="max-w-5xl mx-auto">
          ...
          <div className="flex-1 text-center sm:text-left w-full">
            {editing ? (
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="grid sm:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input type="text" value={form.twitter} onChange={(e) => setForm((p) => ({ ...p, twitter: e.target.value }))}
                      className="w-full bg-(--background) border border-(--border) rounded-xl px-3 py-2 text-sm text-(--foreground) outline-none focus:border-(--primary) focus:ring-2 focus:ring-(--ring)/20" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-(--muted-foreground) mb-1">LinkedIn</label>
                    <input type="text" value={form.linkedin} onChange={(e) => setForm((p) => ({ ...p, linkedin: e.target.value }))}
                      className="w-full bg-(--background) border border-(--border) rounded-xl px-3 py-2 text-sm text-(--foreground) outline-none focus:border-(--primary) focus:ring-2 focus:ring-(--ring)/20" />
                  </div>
                </div>
              ...
              <>
                ...
                <p className="text-(--muted-foreground) text-sm mt-0.5">@{form.username}</p>
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 mt-3 text-xs text-(--muted-foreground)">
                  <span className="flex items-center gap-1.5"><FiMail className="text-(--primary)" /> {user?.email || "john@example.com"}</span>
                  <span className="flex items-center gap-1.5"><FiCalendar className="text-(--primary)" /> Joined {joinedDate}</span>
                  <span className="flex items-center gap-1.5"><FiMapPin className="text-(--primary)" /> {form.location}</span>
                </div>
                <p className="text-sm text-(--foreground) mt-3 max-w-lg">{form.bio}</p>
                <div className="flex items-center gap-4 mt-3 text-xs text-(--muted-foreground)">
                  {form.website && <span className="flex items-center gap-1"><FiGlobe /> {form.website}</span>}
                  {form.twitter && <span className="flex items-center gap-1"><FiTwitter /> {form.twitter}</span>}
                  {form.linkedin && <span className="flex items-center gap-1"><FiLinkedin /> {form.linkedin}</span>}
                </div>
      ...
      <motion.div variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {stats.map((stat) => {
          ...
          return (
            <motion.div key={stat.label} variants={fadeUp} className="bg-(--card) border border-(--border) rounded-2xl p-5 flex items-center gap-4">
          ...
          {filteredSaved.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-xs text-(--muted-foreground)">
                {savedTab === "all" ? "No saved spots yet" : savedTab === "wantToVisit" ? "No spots marked as want to visit" : "No visited spots yet"}
              </p>
            </div>
          ) : (
            <div className="space-y-1">
              {filteredSaved.map((s) => {
                ...
                if (!dest) return null;
                return (
                  <div key={s.destinationId} className="flex items-center gap-3 p-2 rounded-xl hover:bg-(--background) transition-colors">
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 shrink-0 ${
                      ...
                      {s.status === "visited" ? <FiCheckCircle /> : <FiBookmark />}

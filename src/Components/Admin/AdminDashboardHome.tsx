    ...
    (async () => {
      ...
      try {
          ...
          return {
            ...
            role:
              String(item.role || "user")
                .charAt(0)
                .toUpperCase() + String(item.role || "user").slice(1),
        ...
        const newUsersThisWeek = userList.filter((u) => {
          ...
          return created && now - new Date(created).getTime() < 7 * day;
        }).length;
        ...
        const recentPending = pending.slice(0, 3).map((item) => {
          const creatorObj = item.creator as
          ...
          return {
            ...
            submittedBy:
              creatorName || (item.submittedBy as string) || "Unknown",
          };
        });
        setPendingDestinations(recentPending);

        const newDestinationsThisMonth = destList.filter((d) => {
          const created = d.createdAt as string | undefined;
          return created && now - new Date(created).getTime() < 30 * day;
        }).length;

        setGrowth({ newUsersThisWeek, newDestinationsThisMonth });
      } catch {
        if (!mounted) return;
        setTotalUsers(0);
        setModeratorCount(0);
    ...
    <div>
        ...
        {statsCards.map((card) => {
          ...
          const href =
            card.label === "Total Users"
              ? "/admin/users"
              : card.label === "Total Destinations" ||
                  card.label === "Pending Reviews"
                ? "/admin/destinations"
                : card.label === "Moderators"
                  ? "/admin/moderators"
                  : "#";
          return (
            <Link key={card.label} href={href}>
              <motion.div
                variants={fadeUp}
                className="bg-(--card) border border-(--border) rounded-2xl p-5 flex items-center gap-4 cursor-pointer hover:border-(--primary)/30 hover:shadow-md transition-all"
              >
                <div
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-(--background) rounded-xl p-4 text-center">
            <p className="text-2xl font-bold text-(--foreground)">
              {growth.newDestinationsThisMonth}
            </p>
            <p className="text-xs text-(--muted-foreground) mt-1">
          ...
          <div className="space-y-1">
            {pendingDestinations.map((dest) => (
              <Link
                ...
                className="flex items-start gap-3 py-3 border-b border-(--border) last:border-0 hover:bg-(--background)/50 rounded-lg px-2 -mx-2 transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] bg-(--border) text-(--muted-foreground) px-1.5 py-0.5 rounded">
                      {dest.category}
                    </span>

...
import {
  ...
  unsuspendUser,
  setUserRole,
} from "@/src/services/usersCommandService";
...
function mapBackendUser(item: Record<string, unknown>): User {
  ...
  return {
    ...
    role: (String(item.role || "user")
      .charAt(0)
      .toUpperCase() + String(item.role || "user").slice(1)) as User["role"],
    ...
    image,
    suspendedAt: item.bannedAt
      ? new Date(item.bannedAt as string).toLocaleString("en-US", {
          month: "short",
          day: "numeric",
          year: "numeric",
          hour: "2-digit",
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-(--foreground)">
              Change Role:{" "}
              <span className="text-(--primary)">{editingUser.name}</span>
            </h3>
            <button
              type="button"
    ...
    try {
      ...
      if (userId && role !== "User") {
        await authClient.admin.setRole({
          userId,
          role: role.toLowerCase() as "admin" | "user" | ("admin" | "user")[],
        });
      }
      onAdded();
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-(--background) border border-(--border) rounded-xl px-4 py-2.5 text-sm text-(--foreground) outline-none cursor-pointer focus:border-(--primary) focus:ring-2 focus:ring-(--ring)/20"
            >
              <option value="User">User</option>
              <option value="Moderator">Moderator</option>
              <option value="Admin">Admin</option>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-(--primary) hover:bg-(--primary-hover) text-white rounded-xl py-2.5 text-sm font-semibold transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
                  <GlobalLoader variant="spinner" size="sm" /> Creating...
                </>
              ) : (
                "Add User"
              )}
